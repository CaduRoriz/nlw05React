...
export default function Home(props: HomeProps) { //fazer um jeito pra usar um objeto criado para eu formatar a data e pegar data separado da duracao, duracao eh pq tenho que pegar do file
  return (
...
export const getStaticProps: GetStaticProps = async ()=> {
  const { data } = await api.get('episodes', {//_limit=12$_sort_=published&_order-desc => to colocando um limite de retorno de 12 objetos episodio, selecionando pela publicação em ordem decrescente(pra usar essa sintaxe só olhando documentacao e acho que isso só serve ppro server j-son), aqui mudei a forma de passar o caminho e os parametros pelo uso do axios
    params: { //isso aqui nao deu certo eu acho
      _limit: 12,
      _sort: 'published_at',
      _order: 'desc'
    }
  ...
  const episodes = data.map((episode: DataEpisode) => {
    return{
      ...
      publishadAat: format(parseISO(episode.published_at), 'd MMM yy', {locale: ptBR}),
    ...
    const allEpisodes = data.slice(2, data.lenght)
